...
- bounded Nova semantic call effects treat a uniquely resolved standalone call at the top level of the analyzed function or nested brace body to an explicitly `-> !` function as non-fallthrough evidence for `nova.unreachable-code`, value-return obligations, and definite-initialization branch joins; cross-file annotation changes rebind through the exact workspace snapshot, ambiguous targets and compound-expression calls remain conservative, and the effect does not escape its enclosing body
