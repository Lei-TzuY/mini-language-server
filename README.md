...
- conservative Nova function-result inference for uniquely resolved unannotated functions whose value-bearing returns consistently resolve to one bounded Int, String, or Bool type through literals, exact semantic parameter/local references with bounded type knowledge, or acyclic chains of uniquely resolved direct calls; ambiguous targets, cycles, conflicting/unsupported/unknown returns remain unknown, and explicit return annotations are never overridden
