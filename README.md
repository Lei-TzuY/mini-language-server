...
- bounded Nova semantic call effects treat a uniquely resolved standalone call at the top level of the analyzed function or nested brace body as non-fallthrough evidence for `nova.unreachable-code`, value-return obligations, and definite-initialization branch joins when the target is explicitly `-> !` or is an unannotated function conservatively proven never-returning from exact current control flow; inferred effects propagate transitively through acyclic uniquely resolved call chains, explicit non-`!` annotations, syntactic returns, ambiguous targets, cycles without an independent divergence base, and compound-expression calls remain conservative, and the effect does not escape its enclosing body
