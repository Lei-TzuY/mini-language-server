...
- definition, references, document highlights, linked editing ranges, prepareRename, and rename, including uniquely resolved cross-file Nova functions; rename negotiates versioned `WorkspaceEdit.documentChanges` when supported so every edited URI carries the exact captured document version, while legacy clients keep `changes`, and the final Nova product rejects workspace function-name collisions before publishing edits
...
- negotiated `textDocument/diagnostic` and `workspace/diagnostic` pull reports with deterministic result IDs, unchanged-report support, cancellation, and exact diagnostic-snapshot publication guards; the generic server-to-client request substrate tracks outbound JSON-RPC requests/responses, and clients advertising `workspace.diagnostics.refreshSupport` receive coalesced `workspace/diagnostic/refresh` requests only after a cross-file workspace diagnostic transition wins its exact compare-and-commit
...
- negotiated Nova quick fixes for unresolved functions, unresolved local names, argument-count mismatches, argument-type mismatches, return-type mismatches, missing value returns, explicit-local initializer type mismatches, duplicate function/parameter/local declarations, and unsupported unary `+`; eager and lazy-resolved edits use versioned `WorkspaceEdit.documentChanges` when negotiated and preserve legacy `changes` otherwise, while unary-plus repair removes only the exact diagnosed operator and all repairs remain gated on exact diagnostic/workspace snapshots
