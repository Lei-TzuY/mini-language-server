...
- bounded explicit-local validation reports deterministic `nova.local-type` diagnostics when an Int, String, or Bool annotation conflicts with a literal, exact semantic reference, or uniquely resolved same-file/cross-file function call whose bounded result type is known from the exact current workspace snapshot
