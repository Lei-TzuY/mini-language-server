...
- exact-workspace Nova hover type information for explicitly typed parameters, explicitly typed locals, literal-initialized locals, exact-reference local aliases, and bounded uniquely resolved direct function-call initializers when type knowledge is available; direct-call initializer inference ignores masked comment trivia around the call without treating compound expressions as direct calls
- conservative Nova function-result inference for uniquely resolved unannotated functions whose value-bearing returns consistently resolve to one bounded Int, String, or Bool type through literals, exact semantic parameter/local references with bounded explicit type knowledge, local alias chains rooted in bounded direct function-call initializers, or acyclic chains of uniquely resolved direct calls; ambiguous targets, cycles, conflicting/unsupported/unknown returns remain unknown, and explicit return annotations are never overridden
