...
Create/delete/rename preflight covers only editor-owned open identities and bounded detached Nova identities already known to the server. It deliberately does not claim filesystem permission/access checks, unindexed external-resource discovery, remote-provider authority, or module/import path rewrites. After the client commits an operation, `didCreateFiles` and `didDeleteFiles` own closed-index reconciliation while `didRenameFiles` additionally owns open-document rekey/rebuild. Filesystem watchers beyond the negotiated client-mediated provider, remote providers, permission-aware filesystem transactions, and module/import path rewrites remain separate milestones.
