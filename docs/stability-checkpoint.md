...
- remaining filesystem lifecycle surfaces: full ACL/credential-aware or atomic filesystem transactions beyond the bounded local immediate-parent mutation feasibility guard, OS-native or remote watcher providers, full closed-file diagnostic/repair parity (push/remaining advanced data-flow diagnostics plus quick fixes beyond the bounded condition-type and scalar constant-condition/division-zero/conversion-range analysis, unresolved-function, call-site argument-count/type, return/local/assignment type-mismatch, immutable-assignment, missing-return, unreachable-code, and typed-var definite-initialization repairs), and package/module authorities beyond the bounded most-specific-workspace-root `@/`, unique LSP workspace-folder-name `@name/`, captured bare-workspace resolver with deterministic ambiguous-candidate provenance, initialization/runtime-configured precedence over active workspace folders plus bounded explicit external local module-provider roots, exact resolved-edge import-cycle diagnostics, and exact server-known import-path completion—remote providers, package registries/manifests, stable package identities, and guaranteed external filesystem observation remain future work—, wildcard name-merging imports beyond existing bare imports and namespace graphs beyond bounded importer-local `* as ns` qualified calls/member completion/private local alias rename/exact identity plus the cycle-safe explicit local-file star/selective namespace export and cross-module rename chain, namespace rename beyond that bounded explicit local-file graph, alias graph rename beyond bounded private importer rename plus exact relative-file/selective/bare/workspace-root import and explicit/implicit export propagation, richer export/visibility controls beyond bounded private-function visibility plus aliased selective local-file import lists, explicit function export lists, and function-only local-file wildcard re-exports, non-function imported namespaces, and cross-authority module resolution beyond the bounded local-file import rewrite, exact document-link navigation, and import-graph function-visibility phase remain future executable phases; bounded unopened local-file indexing, exact open+detached `willCreateFiles` / `willDeleteFiles` / `willRenameFiles` preflight with bounded unindexed local path-entry evidence, negotiated create/delete/rename reconciliation, dynamically registered client-mediated watched-file reconciliation, and pull-only workspace diagnostics are now part of the stable workspace substrate
