...
- remaining filesystem lifecycle surfaces: `willRenameFiles` still preflights only open-document ownership, while filesystem watchers or remote providers, closed-file diagnostics, and module/import rewrite semantics remain future executable phases; bounded unopened local-file indexing plus negotiated create/delete/rename-triggered reconciliation are now part of the stable workspace substrate
