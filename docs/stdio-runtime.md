...
3. while that request is active, `textDocument/didOpen`, `didChange`, `didClose`, negotiated `workspace/didChangeWorkspaceFolders`, `workspace/didChangeConfiguration`, dynamically registered `workspace/didChangeWatchedFiles`, negotiated `workspace/didCreateFiles`, `workspace/didDeleteFiles`, `workspace/didRenameFiles`, valid responses to already-delivered pending server-to-client requests, and a coalesced server-request-outbox wakeup may advance on the foreground thread so exact snapshot/configuration guards and bidirectional request dependencies can observe real transport-time input;
