...
This outbound-request substrate is intentionally generic. Diagnostic and CodeLens refreshes already reuse the same tracked lifecycle independently, and future semantic-token, inlay-hint, or other refresh methods should add only their own capability negotiation and conservative invalidation policy rather than creating feature-specific request queues.
