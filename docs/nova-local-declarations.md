...
The bounded analysis also proves structurally complete `if`/`else if`/`else` joins when every arm that can reach the later read definitely initializes the same exact local symbol. An arm may establish that proof through a direct assignment, recursively through its own structurally complete nested `if`/`else if`/`else` chain, or by an unconditional `return` statement in that arm's direct lexical scope because that path cannot reach the join. A return hidden only in a nested conditional does not terminate its enclosing arm. A missing final `else`, a reachable missing assignment path in any nested or outer arm, or a loop-only assignment does not prove the join. The adapter intentionally remains conservative for loops and other control-flow cases that require a fuller control-flow graph.
