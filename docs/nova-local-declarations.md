...
The bounded analysis also proves a plain `if`/`else` join when both direct arms contain a resolved assignment to the same exact local symbol before the later read. A missing assignment in either arm remains uninitialized, and an assignment hidden inside a deeper nested conditional does not prove its containing arm. The adapter intentionally remains conservative for `else if`, loops, early exits, and other cases that require a fuller control-flow graph.
