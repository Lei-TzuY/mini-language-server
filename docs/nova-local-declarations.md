...
The bounded analysis also proves structurally complete `if`/`else if`/`else` joins when every arm definitely initializes the same exact local symbol before the later read. An arm may establish that proof through a direct assignment or recursively through its own structurally complete nested `if`/`else if`/`else` chain. A missing final `else`, a missing assignment path in any nested or outer arm, or a loop-only assignment does not prove the join. The adapter intentionally remains conservative for loops, early exits, and other cases that require a fuller control-flow graph.
