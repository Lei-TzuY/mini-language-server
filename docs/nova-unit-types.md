...
Unannotated function-result inference also treats direct bare `return;` statements as `Unit`. A function whose observed returns are all bare returns, or a mixture of bare returns and value returns already proven to be `Unit`, can therefore contribute `Unit` through the existing exact-workspace hover/completion/signature/inlay and type-diagnostic consumers. A mixture of bare returns with any non-`Unit`, conflicting, unsupported, ambiguous, or cyclic value-return inference remains unknown rather than inventing a result type. A function with no return statements is not inferred as `Unit` merely because it can fall through.
