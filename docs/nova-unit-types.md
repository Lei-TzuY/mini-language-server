...
Unannotated function-result inference also treats reachable direct bare `return;` statements as `Unit` evidence. Returns inside branches proven dead by bounded constant-condition facts are excluded before inference, so a dead value return does not poison an otherwise reachable all-bare result. If any reachable value-bearing return remains alongside a bare return, result inference stays unknown; unknown conditional branches are not pruned. A function with no reachable return statements is likewise not inferred as `Unit` merely because it can fall through.
