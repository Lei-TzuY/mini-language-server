...
Unannotated function-result inference also treats reachable direct bare `return;` statements as `Unit` evidence. Returns inside branches proven dead by bounded constant-condition facts, and returns in same-scope suffixes after a proven non-fallthrough loop, are excluded before inference. If any reachable value-bearing return remains alongside a bare return, result inference stays unknown; unknown conditional branches and loops with reachable or possibly reachable breaks are not pruned. A function with no reachable return statements is likewise not inferred as `Unit` merely because it can fall through or diverge.
