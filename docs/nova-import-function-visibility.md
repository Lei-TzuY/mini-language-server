...
- an importer-local namespace `import * as api from ./path.nova;` or `import * as api from @/path.nova;` exposes that target module's exact outward function view locally only as qualified `api::member()` calls; a module may explicitly `export { api };` when `api` is one unique non-reserved namespace binding with no same-named function binding, projecting that target's exact outward function view into the module's own outward view for downstream import-graph consumers without making `api` a plain local function binding; duplicate namespace aliases and namespace-vs-function export collisions fail closed, and `Int` / `UInt` remain reserved;
...
- wildcard name-merging import syntax beyond existing bare imports, nested/package namespace objects, selective namespace imports, or cross-module namespace-object identity beyond the bounded `export { ns }` outward-function projection;
