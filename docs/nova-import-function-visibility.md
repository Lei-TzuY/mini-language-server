...
- wildcard imports or outward/re-exported alias graph rename;
...
Transitive visibility is intentionally limited to function declarations reachable through existing bare or selective relative-file import edges plus the bounded private-function and explicit export-list rules above; it does not imply package identity, wildcard namespaces, exported alias API propagation, or non-function exports; only importer-private aliases excluded from the outward export view are independently renameable in this phase. Specialized later product layers that are not part of the shared call-resolution surfaces above remain future namespace-parity work and must not infer broader module semantics from this bounded contract.
