...
- an importer-local namespace `import * as api from ./path.nova;` or `import * as api from @/path.nova;` exposes that target module's exact outward function view locally only as qualified `api::member()` calls; a module may explicitly `export { api };` when `api` is one unique non-reserved namespace binding with no same-named function binding, projecting that exact namespace object plus its target outward function view across one module boundary; a downstream selective `import { api as facade } from ./middle.nova;` may then install one importer-local namespace identity whose qualified `facade::member()` calls resolve against the original provider view, while a same-named exported function makes the selection ambiguous and fails closed; duplicate/reserved namespace bindings fail closed, and `Int` / `UInt` remain reserved;
...
- wildcard name-merging import syntax beyond existing bare imports, nested/package namespace objects, selective namespace re-export/chaining beyond one direct exported-namespace selection, or cross-module namespace-object identity beyond the bounded direct `export { ns }` → selective-import graph;
