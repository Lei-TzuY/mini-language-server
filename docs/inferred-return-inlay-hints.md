...
Each published inferred-return hint is directly actionable: its LSP `textEdits` inserts the same exact ` -> Type` annotation immediately after the declaration's parameter list, so clients that support applying inlay-hint edits can materialize the inferred type without recomputing it independently.
...
- the edit and displayed label are derived together from the same inferred type and zero-width post-parameter insertion range;
...
The insertion anchor is discovered from the trivia-masked Nova code view, preserving source offsets while ignoring parentheses inside comments and quoted strings.
