...
The rule is intentionally conservative. Functions with any reachable explicit return and a tail expression are not inferred by this layer; returns under unknown conditional branches remain reachable evidence. Ambiguous or unsupported tails, incomplete bodies, recursive cycles, and stale workspace snapshots remain unknown. Existing explicit-return and bare-return inference behavior remains unchanged when no tail expression competes with it.
