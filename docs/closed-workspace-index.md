...
Closed snapshots never publish push diagnostics and are not inserted into the live DiagnosticStore. They do participate in negotiated `workspace/diagnostic` pulls through ephemeral immutable diagnostic snapshots: stable duplicate function/parameter/local declaration diagnostics are retained from the detached parse, while function-call errors are recomputed against the exact captured combined workspace for unresolved/ambiguous targets and bounded argument-count mismatches on uniquely resolved calls. Adapter-level unresolved-name output and later product-only argument-type, return/type, local-type, and data-flow diagnostics remain fail-closed until their full policy can be reproduced without closed-only false positives. Closed reports use `version: null` and deterministic `null:<digest>` result IDs, so no detached internal version is presented as a filesystem concurrency claim. Their declarations and parse-tree call sites also participate in existing workspace symbol search, definition/hover/completion, reference scans, call hierarchy, collision checks, and cross-file call resolution for open documents.
