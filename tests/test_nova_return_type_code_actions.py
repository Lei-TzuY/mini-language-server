    ...
    server.handle(
        notify(
            "textDocument/didChange",
            {
                ...
                "contentChanges": [
                    {"text": "fn target(value: Int) -> Int { return value }\n"}
                ],
            },
        )
    )
    assert code_action(server, uri, 3, start, start + len("value"))["result"] == []


class ReplacingSemanticServer(NovaProductLanguageServer):
...
def test_same_version_semantic_replacement_suppresses_stale_return_quick_fix() -> None:
    server = ReplacingSemanticServer()
    result = server.handle(
        request(
            "initialize",
            1,
