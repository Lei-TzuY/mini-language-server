...
def test_initialize_advertises_implementation_navigation() -> None:
    capabilities = initialize(NovaProductLanguageServer())["result"]["capabilities"]
    assert capabilities["implementationProvider"] is True


def test_implementation_reuses_exact_cross_file_binding() -> None:
    server = NovaProductLanguageServer()
    initialize(server)
    ...
    result = implementation(server, caller_uri)
...
def test_implementation_refuses_ambiguous_workspace_function() -> None:
    server = NovaProductLanguageServer()
    initialize(server)
    caller_uri = "file:///workspace/main.nova"
    ...
    assert implementation(server, caller_uri)["result"] is None


def test_implementation_tracks_close_and_reopen() -> None:
    server = NovaProductLanguageServer()
    initialize(server)
    target_uri = "file:///workspace/library.nova"
    caller_uri = "file:///workspace/main.nova"
    open_nova(server, target_uri, "fn target() {}\n")
    open_nova(server, caller_uri, "fn caller() { target() }\n")
    assert implementation(server, caller_uri)["result"] is not None

    server.handle(notify("textDocument/didClose", {"textDocument": {"uri": target_uri}}))
    assert implementation(server, caller_uri, 3)["result"] is None

    open_nova(server, target_uri, "fn target() {}\n", version=1)
    assert implementation(server, caller_uri, 4)["result"] is not None


def test_implementation_rejects_same_version_workspace_replacement() -> None:
    server = NovaProductLanguageServer()
    initialize(server)
    target_uri = "file:///workspace/library.nova"
    caller_uri = "file:///workspace/main.nova"
    ...
    assert implementation(server, caller_uri) == {
        "jsonrpc": "2.0",
        "id": 2,
...
def test_implementation_honors_request_cancellation_before_publication() -> None:
    server = NovaProductLanguageServer()
    initialize(server)
    target_uri = "file:///workspace/library.nova"
    caller_uri = "file:///workspace/main.nova"
    ...
    assert implementation(server, caller_uri, 9) == {
        "jsonrpc": "2.0",
        "id": 9,
