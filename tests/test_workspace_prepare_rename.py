...
def initialize(server: WorkspaceNovaLanguageServer) -> None:
    result = server.handle(request("initialize", 1, {}))
    assert result is not None
    assert result["result"]["capabilities"]["renameProvider"] == {
...
def prepare(server: WorkspaceNovaLanguageServer, uri: str, request_id: int = 2) -> dict:
...
def test_workspace_prepare_rename_resolves_unique_cross_file_function() -> None:
    ...
    assert prepare(server, main_uri)["result"] == {
        "range": {
...
def test_workspace_prepare_rename_does_not_guess_ambiguous_function() -> None:
    server = WorkspaceNovaLanguageServer()
    initialize(server)
    main_uri = "file:///workspace/main.nova"
    ...
    assert prepare(server, main_uri)["result"] is None


def test_workspace_prepare_rename_tracks_change_and_close_reopen() -> None:
    server = WorkspaceNovaLanguageServer()
    initialize(server)
    ...
    open_nova(server, main_uri, "fn caller() { target() }\n")
    assert prepare(server, main_uri)["result"] is not None
    ...
    assert prepare(server, main_uri, 3)["result"] is None

    server.handle(
        notify("textDocument/didClose", {"textDocument": {"uri": library_uri}})
    )
    open_nova(server, library_uri, "fn target() {}\n")
    assert prepare(server, main_uri, 4)["result"] is not None


def test_workspace_prepare_rename_suppresses_same_version_replacement() -> None:
    server = WorkspaceNovaLanguageServer()
    initialize(server)
    library_uri = "file:///workspace/library.nova"
    main_uri = "file:///workspace/main.nova"
    ...
    assert prepare(server, main_uri) == {
        "jsonrpc": "2.0",
        "id": 2,
